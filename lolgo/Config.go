...
func (cfg *Config) Parse(fpath string) {
	...
	defer fi.Close() //关闭文件操作指针
	br := bufio.NewReader(fi)  //读取到缓存中

	for {
		a, _, c := br.ReadLine()   //一行一行读取
		if c == io.EOF {  //读取到的字符串为空的时候停止
			break
		}
		tmp := strings.TrimLeft(string(a), " ") //去除左边的空格
		tmp = strings.TrimRight(tmp, " ")   //去除右边的空格
		if len(tmp) == 0 || strings.Index(tmp, "#") == 0 {  //"#"开头的信息
			continue
		}

		o := strings.Split(tmp, "=")   //以"="分割
		if len(o) == 2 {
			cfg.All[o[0]] = o[1]
		} else if len(o) < 2 {
			cfg.All[o[0]] = ""
		} else {
			cfg.All[o[0]] = strings.TrimPrefix(tmp, o[0]+"=") //去除字符串的前缀 前缀自定义
